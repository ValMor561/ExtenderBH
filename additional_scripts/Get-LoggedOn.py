...
class RemoteOperations:
    ...
    def connectWinReg(self):
        try:
            ...
            logging.debug("Trying to start the Remote Registry...")    
...
class Check:
    ...
    def run(self, remoteName, remoteHost):
        ...
        logging.debug('Cannot check RemoteRegistry status. Hoping it is started...')
        ...
        try:
            ...
            if resp['TranslatedNames']['Names'] == []:
                logging.info(remoteName)
            else:
                strlist = ''
                for item in resp['TranslatedNames']['Names']:
                    try:
                        if item['Use'] != SID_NAME_USE.SidTypeUnknown:
                            strlist += f" [{item['Name'].lower()}@{resp['ReferencedDomains']['Domains'][item['DomainIndex']]['Name'].lower()}]"
                    except:
                        pass
                logging.info(f"{remoteName}:{strlist}")
...
if __name__ == '__main__':
    ...
    if options.target_file is not None:
        ...
        remote_names.append(remoteName)
    ...
    # print(ipaddress.IPv4Address('192.0.2.5')) 
    for remote_name in remote_names:
        try:
            options.target_ip = remote_name
            Check(username, password, domain, options).run(remote_name, remote_name)
            # print()
